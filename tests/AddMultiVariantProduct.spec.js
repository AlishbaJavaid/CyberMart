...
const flavorPool = ['Mango', 'Strawberry', 'Chocolate', 'Vanilla', 'Banana', 'Blueberry', 'Coconut', 'Pineapple', 'Lemon', 'Caramel'];

// Function to pick a random flavor
function getRandomFlavor() {
  const randomIndex = Math.floor(Math.random() * flavorPool.length);
  return flavorPool[randomIndex];
}

// Generate a random flavor
const randomFlavor = getRandomFlavor();

// Fill flavor and click Add
await page.getByRole('textbox', { name: 'Flavor' }).click();
await page.getByRole('textbox', { name: 'Flavor' }).fill(randomFlavor);
await page.getByRole('button', { name: 'Add' }).first().click();

// List of possible pack sizes
const packSizePool = ['Small', 'Medium', 'Large', 'Extra Large', 'Mini', 'Regular', 'Family', 'Single'];

// Function to pick a random pack size
function getRandomPackSize() {
  const randomIndex = Math.floor(Math.random() * packSizePool.length);
  return packSizePool[randomIndex];
}

// Generate a random pack size
const randomPackSize = getRandomPackSize();

// Fill pack size and click Add
await page.getByRole('textbox', { name: 'Enter Pack Size' }).click();
...
// Fill first color and click Add
await page.getByRole('textbox', { name: 'Enter Color' }).click();
await page.getByRole('textbox', { name: 'Enter Color' }).fill(randomColor1);
await page.getByRole('button', { name: 'Add' }).nth(2).click();

// Fill second color and click Add
await page.getByRole('textbox', { name: 'Enter Color' }).click();
await page.getByRole('textbox', { name: 'Enter Color' }).fill(randomColor2);
await page.getByRole('button', { name: 'Add' }).nth(2).click();
...
function getRandomSKU(length = 8) {
  ...
  return 'SKU-' + sku; // Prefix to make it clear and readable
}

// Generate unique random SKU
const randomSKU = getRandomSKU(8);
...
// Click on Promo Price input for the correct row
await page.getByRole('row', { name: new RegExp(productRowName) })
  .getByPlaceholder('Enter Promo Price').click();

// Fill Promo Price
await page.getByRole('row', { name: new RegExp(productRowName) })
  .getByPlaceholder('Enter Promo Price').fill('50');

// Open calendar only once
await page.getByRole('row', { name: new RegExp(productRowName) })
  .getByTestId('CalendarMonthIcon').click();
